<!-- TODO: 
	- There are still some links to property.php instead of admin_property.php, double
		check that these are all correct
	- Right now anyone can reply to a comment - This should only appear when the user owns the property
-->
...
<div class="page-header">
  <h1>Admin: Property Details</h1>
		...
		<?php
			...
			if(isset($_GET['confirm_booking_btn'])) {
				// Necessary  Data is provided
				if(isset($_GET['property_id']) && isset($_GET['date']) && !empty($_GET['date']))
				{
			...
			} elseif(isset($_GET['confirm_delete_btn'])) { // Delete has been confirmed
				...
				$query = "SELECT member_id FROM supplies WHERE property_id=?;";
				$parm1 = $_GET['property_id'];
				$stmt = $con->prepare($query);
				$stmt->bind_Param('s', $parm1);
				$stmt->execute();
				$ownerRslt = $stmt->get_result();
				$owner = $ownerRslt->fetch_assoc();
				
				// Notify owner:
				$query = "INSERT INTO notification values (NULL, ?, ?, FALSE);";
				$parm1 = $owner["member_id"];
				$parm2 = "Your property #" . $_GET["property_id"] . " was removed by an administrator.";
				$stmt = $con->prepare($query);
				$stmt->bind_Param('ss', $parm1, $parm2);
				$stmt->execute();
				...
				echo "Property # " . $_GET['property_id'] . "sucessfully deleted.";
				die();
			} elseif(isset($_GET['booking_btn'])) { // Book Now has been clicked but not confirmed
		?>
				<form class="form-inline" action='admin_property.php' method=get>
					...
					<input class="btn btn-success"  name='confirm_booking_btn' id='confirm__booking_btn' value="Confirm" type="submit">
				</form>
		<?php
			} elseif(isset($_GET['delete_btn'])) { // 'Delete' has been clicked but not confirmed
		?>
				<form class="form-inline" action='admin_property.php' method=get>
					<label for="confirm_booking_btn">Are you sure you want to delete this property?</label>
					<a href="admin_property.php?property_id=<?php echo $prop_id ?>" class="btn btn-default" role="button">Cancel</a>
					<input type='hidden' name='property_id' id='property_id' value="<?php echo $prop_id ?>">
		...
		?>
				...
				<input class="btn btn-danger" name='delete_btn' id='delete_btn' value="Delete Property" type="submit">
				<input type='hidden' name='property_id' id='property_id' value="<?php echo $prop_id?>">
				</form>
		<?php
			}
		...
		if($_SESSION['id']){
					...
					$prop_id = $_GET['property_id'];
		 			// SELECT query
						...
						WHERE
						property_id = '$prop_id' AND DATE_ADD(start_date, INTERVAL 7 day) >= CURRENT_DATE;
		 						  ;"; 
						...
						property_id = '$prop_id' AND DATE_ADD(start_date, INTERVAL 7 day) < CURRENT_DATE;"; 
		 				...
		 				if ($result->num_rows > 0) {
					        ...
					        echo "</table></div>";
		 				     ...
		 				     echo "No past bookings on this property.";
		 				}
				}			
				?>
				        ...
				        while($row = $result->fetch_assoc()) {
		 				...
		 				if ($result->num_rows > 0) {
					        echo "<div class=\"table-responsive\">
								 <table class=\"table\">
								 <head>
									<tr><th>Comment Id</th><th>Member Id</th><th>Rating</th><th>Comment</th><th>Time</th><th>Reply</th>";
							echo "</tr>";
							echo "</head>";
					        // output data of each row
					        while($row = $result->fetch_assoc()) {
